...
- 默认 Brand 主色保持蓝紫体系；Absolutely 主题使用独立的暖陶土色与暖中性色，不改变状态色的职责。
- 绿色只表示成功、完成或健康状态，不作为普通数据主色。
- 默认调色板的橙色只表示注意、进行中或输出类数据，不作为危险色；Absolutely 主题的陶土色属于 Brand 色，不承担状态语义。
- 红色只表示错误、危险或不可继续。
- 图表色阶使用同一语义色族的明度变化，避免彩虹色。
- 进度填充使用同一蓝紫色族时，从浅色起点平滑过渡到深色终点。
- Absolutely 主题的主强调色使用 `#CC785C`，表面使用 `#F0EFEA`、`#F7F6F2` 等暖中性色；所有变体仍必须集中在统一 palette 中。
- Absolutely 主题的数据拆分与价值进度使用浅陶土、主陶土和暖棕色阶，不混入默认主题的蓝紫色阶。
...
- 使用系统字体和系统文本层级。
- Absolutely 主题中文优先使用 `Noto Serif CJK SC`；英文、数字和代码按 `Fira Code`、`Fira Mono`、`Menlo`、`Consolas`、`DejaVu Sans Mono` 的顺序选择首个可用等宽字体，全部不可用时回退到系统等宽字体。
